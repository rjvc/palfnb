...
function scene1(p) {
  ...
  const yo = (1-easeOutQuart(tp))*50;
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.font = `700 ${68}px Montserrat`;
  ctx.fillStyle = COLORS.branco; ctx.globalAlpha = easeOutQuart(tp);
  ctx.fillText('GRUPO CASTRO', W/2, H/2-50+yo);
  ctx.font = `400 ${40}px Montserrat`;
  ctx.fillStyle = COLORS.dourado;
  ctx.fillText('CONSULTORIAS', W/2, H/2+20+yo);
  const lp = p>0.4 ? Math.min(1, (p-0.4)*3) : 0;
  goldLine(ctx, W/2-150, H/2+80+yo, 300, lp);
  ctx.globalAlpha = 1;
  return c;
}

// Scene 2: Event title (5–12s)
function scene2(p) {
  const c = createCanvas(); const ctx = c.getContext('2d');
  ...
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.font = `400 ${32}px Montserrat`;
  ctx.fillStyle = COLORS.verdeMenta; ctx.globalAlpha = easeOutQuart(sp);
  ctx.fillText('APRESENTA', W/2, H/2-200+sy);
  const tp = p>0.25 ? Math.min(1, (p-0.25)*2) : 0;
  const ty = (1-easeOutQuart(tp))*60;
  ctx.font = `900 ${160}px Montserrat`;
  ctx.fillStyle = COLORS.branco; ctx.globalAlpha = easeOutQuart(tp);
  ctx.fillText('F&B', W/2-60, H/2-40+ty);
  ctx.fillStyle = COLORS.dourado;
  ctx.fillText('4.0', W/2+150, H/2-40+ty);
  ...
  ctx.fillText('A Nova Era da Gestão', W/2, H/2+80+dy);
  const lp = p>0.6 ? Math.min(1, (p-0.6)*3) : 0;
  goldLine(ctx, W/2-220, H/2+130+dy, 440, lp);
  ctx.globalAlpha = 1;
  return c;
}

// Scene 3: Event details (12–18s)
function scene3(p) {
  const c = createCanvas(); const ctx = c.getContext('2d');
  drawBg(ctx, COLORS.azulEscuro, '#0a1220');
  const p1 = p>0.1 ? Math.min(1, (p-0.1)*2) : 0;
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.font = `700 ${44}px Montserrat`;
  ...
  ctx.fillText('10 de Setembro de 2026', W/2, H/2-50+y2);
  const p3 = p>0.5 ? Math.min(1, (p-0.5)*2) : 0;
  const y3 = (1-easeOutQuart(p3))*40;
  ctx.font = `400 ${34}px Lora`;
  ctx.fillStyle = COLORS.bege; ctx.globalAlpha = easeOutQuart(p3);
  ctx.fillText('Morro Bento Business Center', W/2, H/2+50+y3);
  ctx.fillText('Luanda · 09:00', W/2, H/2+100+y3);
  ctx.globalAlpha = 1;
  return c;
}
...
function scene4(p) {
  ...
  ctx.beginPath(); ctx.roundRect(-400, -220, 800, 440, 20); ctx.fill(); ctx.stroke();
  ctx.restore();
  const p1 = p>0.25 ? Math.min(1, (p-0.25)*2) : 0;
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.font = `700 ${36}px Montserrat`;
...
function scene5(p) {
  ...
  ctx.fillText('#F&B4ponto0', W/2, H/2+120+y3);
  ctx.globalAlpha = 1;
  return c;
}
...
async function generate() {
  ...
  const framesDir = path.join(__dirname, 'video-teaser', 'frames-sq');
  ...
  const outPath = path.join(outDir, 'teaser-fb4-square.mp4');
