...
const FONTS_DIR = '/paperclip/.claude/skills/canvas-design--f68a068779/canvas-fonts';

// Register fonts
registerFont(path.join(FONTS_DIR, 'BricolageGrotesque-Regular.ttf'), { family: 'Bricolage' });
registerFont(path.join(FONTS_DIR, 'Lora-Regular.ttf'), { family: 'Lora' });
registerFont(path.join(FONTS_DIR, 'WorkSans-Regular.ttf'), { family: 'WorkSans' });
registerFont(path.join(FONTS_DIR, 'NationalPark-Bold.ttf'), { family: 'NationalPark' });
...
const COLORS = {
  ...
  muted: '#4a4a6a',        // Muted purple-gray
};
...
function createRollupPrincipal() {
  ...
  // Logo area
  ctx.fillStyle = COLORS.secondary;
  ctx.font = 'bold 42px NationalPark';
  ...
  ctx.fillText('[Data e local a definir]', 120, 820);
  ...
  ctx.fill();
  ctx.fillStyle = COLORS.secondary;
  ctx.font = 'bold 48px NationalPark';
...
function createRollupInformativo() {
  ...
  // Date/Location
  ctx.fillStyle = COLORS.secondary;
  ctx.font = 'bold 36px NationalPark';
  ctx.fillText('[Data]  |  [Local]  |  Luanda', W/2 + 150, 650);
...
function createSocialPost(type, format) {
  ...
  // Logo
  ctx.fillStyle = COLORS.secondary;
  ctx.font = `bold ${W > 1500 ? 36 : 28}px NationalPark`;
  ...
  // CTA
  ctx.fillStyle = COLORS.accent;
  ctx.font = `bold ${bodySize}px NationalPark`;
  ctx.fillText('[Data e local a definir]', 40, H - 120);
